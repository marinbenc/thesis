...
however, their approach focuses on image classification, not segmentation. In addition, our approach uses two separate neural networks, which allows us to better focus on improving data efficiency.
...
Given a polar origin $(c_x, c_y)$ of a Cartesian image $I(x, y)$ of resolution $H \times W$, we obtain each point $(r, \vartheta)$ of the polar transformed image $I'(r, \vartheta)$ as:
  \begin{equation}
    \begin{aligned}
      r &= \frac{H}{2 \pi} \cdot \mathrm{atan2}(x - c_x, y - c_y) [ \cdot 180 / \pi ], \\
      \vartheta &= \frac{W}{\sqrt{(W / 2)^2 + (H / 2)^2}} \cdot \sqrt{(x - c_x)^2 + (y - c_y)^2}
    \end{aligned},
...
To validate segmentation accuracy and data efficiency improvements of our method, we trained a variety of neural network architectures on 
...
We used four different datasets to train the network. In this section, we give an overview of each used dataset and how it was preprocessed. Note that for training the center-point predictor, the input images were resized to a resolution of $256 \times 256$ to improve training times, while the generated heatmaps were resized to $64 \times 64$ pixels to improve computational requirements. Otherwise, all preprocessing steps described here are applied to the center-point model datasets as well. Each dataset was globally normalized using predetermined Hounsfield ranges for relevant tissues as minimum and maximum values, and zero-centered based on the mean intensity across the training dataset, to facilitate better network convergence.
...
& Model centers (Ours) & 0.9374 & 0.8977 & 0.9488 & 0.9368 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9332 & 0.8924 & 0.9477 & 0.9321 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9338 & 0.8967 & 0.9436 & 0.9347 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9224 & 0.8699 & 0.9165 & 0.9494 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9253 & 0.8743 & 0.9253 & 0.9464 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9235 & 0.8721 & 0.9125 & 0.9570 \\ [1ex]
\end{tabularx}
...
& Model centers (Ours) & 0.9125 & 0.8828 & 0.9108 & 0.9219 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9109 & 0.8795 & 0.9009 & 0.9306 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.9092 & 0.8783 & 0.9075 & 0.9199 \\ [1ex]
\hline \\ [-1.5ex]

\multicolumn{6}{c}{\textbf{EAT Segmentation}}\\[1ex]
 \hline
 Segm. net. & Method & DSC & mIoU & Prec. & Rec. \\ 
 \hline \\ [-1.5ex]
 \multirow{4}{7em}{{U-Net}}
...
& Model centers (Ours) & 0.7840 & 0.6252 & 0.7451 & 0.7302 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.7740 & 0.6140 & 0.7156 & 0.7453 \\ [1ex]
\hline \\ [-1.5ex]
...
& Model centers (Ours) & 0.6720 & 0.4779 & 0.6070 & 0.6488 \\ [1ex]
\end{tabularx}
...
\end{tabularx}
\caption{A comparison between our method (model centers approach with best underlying architecture) and the state of the art on the same datasets.}
...
\begin{enumerate}
	\item Our transformation predictor network will be enhanced to predict a set of transformation parameters \(\{\phi_1, \phi_2, \cdots, \phi_n\}\), where \(n\) varies based on the image and represents the number of transformations. $n$ is dynamically chosen by the network and corresponds to the predicted number of objects in the image.
	\item For each transformation parameter in $\phi_i$ we will produce a corresponding transformed image $I'_i = \mathcal{T}_{\phi_i}(I)$.
